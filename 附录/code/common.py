# 共享模型核心：单日/全年 LP、无储能与规则套利基线、Q3 滚动 MPC。
# 约定T=144, Δt=1/6h；x 计划购电, c 充电, q 放电, E 储能(kWh)。
import os
import pickle
...
SEED = 2026
np.random.seed(SEED)#固定随机种子保证结果可复现
...
# 报告期: 2025-02-01 ~ 2025-12-31, 365 天中索引 31..364 (1 月为预热, HD5)
WARMUP_DAYS = 31
REPORT_DAYS = 334
...
def report_dates(bundle):
    """报告期 334 天 datetime 列表 (与结果模板 日期 列一致)。"""
    ds = bundle["dates"]
    out = []
...
def daily_lp(price, load, pv, E0=None, terminal_eq=False, E_term=None):
    """单日 LP。返回 (cost, x, c, q, E), x/c/q 长度 T, E 长度 T+1。
...
def no_storage_dayahead(price, load, pv_actual, forecast, d):
    ...
    0:00 计划 x = max(L·Δt − ghat0·Δt, 0); 紧急 z = max(min(ghat0,L) − G实际, 0)·Δt。"""
...
def rule_arbitrage(price, load, pv, E0=EMIN, terminal_eq=False, E_term=None):
    """规则峰谷套利基线 (贪心时序模拟)。低价 1/3 充满、高价 1/3 放空、中价不动。"""
...
def forecast_intervals(forecast, d, s_idx):
    """附件3 预报 -> 144 区间光伏功率(kW)。issue s_idx∈{0,1,2,3}=0/6/12/18 点。"""
    ghat = np.zeros(T)
    issue_hour = 6 * s_idx
...
def pv_overestimation(forecast, pv):
    """各 (天, issue) 光伏高估 o = Ĝ − G (10min 区间, 每块 36 区间)。返回 (D, 4, 36)。"""
    D = pv.shape[0]
...
def issue_risk_delta(over, d, quantile=80.0, min_hist=7):
    """day d 各 issue 保守裕量 δ[s]=P_q(光伏高估), 因果(仅用 [min_hist, d) 历史)。"""
    delta = np.zeros(4)
    if d <= min_hist:
        return delta
...
def stage_lp(price, load, pv, x_plan, E0, terminal_eq=False, E_term=None):
    """Q3 调整阶段 LP (给定 0:00 计划 x_plan, 求剩余时段最优调整 y)。
    分段计费线性化: y = u + v, 0≤u≤x_plan(计划内), v≥0(溢价); 费用 = p·[0.5u+1.5v] (常数项略)。
    terminal_eq=True 固定 E_M=E_term(缺省 E0), 实现日闭合 E(144)=6000。返回 (cost_var, y, c, q, E)。"""
    M = len(price)
    n = 4 * M + (M + 1)
...
def rule_mpc_day(price, load_d, pv_actual, forecast, d, E_carry, delta=None):
    """Q3 基线 R3 (规则套利 + 滚动预报): 与 M3 同信息、同计费, 仅用贪心规则替代 LP。
    返回 (cost, x, y_final, c_final, q_final, E_end, z)。"""
    delta = np.zeros(4) if delta is None else np.asarray(delta, dtype=float)
    ghat0 = clip0(forecast_intervals(forecast, d, 0) - delta[0])
...
# 5) 结果写盘 (加载模板填充; 骨架 sheet 重建为完整 334 天)
