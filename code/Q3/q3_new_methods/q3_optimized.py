...
def mpc_day(price, load_d, pv_actual, fc, d, E_carry, delta):
    ...
    # 日内实际执行储能轨迹（4 段拼接，长度 T+1），用于校验 1200≤E≤10800 与日闭合 E0/E24。
    E_day = np.concatenate([E0t[:37], E1t[1:37], E2t[1:37], E3t[1:37]])
    return float(bill.sum()+ALPHA_EM*(price*z).sum()), x, y, c, qv, E3t[-1], z, E_day


def static_day_block(price, load_d, pv_actual, fc, d, E_carry, delta):
    """基线 B3（静态不调整）的块级修正版：仅 0:00 计划并执行，误差由紧急购电吸收。"""
    g0 = corrected_forecast(fc, d, 0, delta)
    _, x, c, q, Et = daily_lp(price, load_d, g0, E0=E_carry, terminal_eq=True, E_term=E0_INIT)
    z = clip0(load_d*DT + c - x - pv_actual*DT - q)
    return float((price*x).sum() + ALPHA_EM*(price*z).sum()), Et[-1], z


def rule_day_block(price, load_d, pv_actual, fc, d, E_carry, delta):
    """基线 R3（规则套利 + 滚动预报）的块级修正版：与 M3 同信息、同计费，仅用贪心规则替代 LP。"""
    g0 = corrected_forecast(fc, d, 0, delta)
    _, x, c0, q0, E0t = rule_arbitrage(price, load_d, g0, E0=E_carry, terminal_eq=True, E_term=E0_INIT)
    g1 = corrected_forecast(fc, d, 1, delta); sl = 36
    _, y1, c1, q1, E1t = rule_arbitrage(price[sl:], load_d[sl:], g1[sl:], E0=E0t[sl], terminal_eq=True, E_term=E0_INIT)
    g2 = corrected_forecast(fc, d, 2, delta); sl = 72
    _, y2, c2, q2, E2t = rule_arbitrage(price[sl:], load_d[sl:], g2[sl:], E0=E1t[sl-36], terminal_eq=True, E_term=E0_INIT)
    g3 = corrected_forecast(fc, d, 3, delta); sl = 108
    _, y3, c3, q3, E3t = rule_arbitrage(price[sl:], load_d[sl:], g3[sl:], E0=E2t[sl-72], terminal_eq=True, E_term=E0_INIT)
    y = np.concatenate([x[:36], y1[:36], y2[:36], y3[:36]])
    c = np.concatenate([c0[:36], c1[:36], c2[:36], c3[:36]])
    qv = np.concatenate([q0[:36], q1[:36], q2[:36], q3[:36]])
    z = clip0(load_d*DT + c - y - pv_actual*DT - qv)
    bill = price*(np.minimum(x, y)+BETA_PEN*np.maximum(x-y, 0)+GAMMA_PRE*np.maximum(y-x, 0))
    return float(bill.sum()+ALPHA_EM*(price*z).sum()), E3t[-1], z
...
def run(b, quantile):
    ...
    E0_min=E0_max=E24_min=E24_max=None
    for d in range(D):
        delta=block_delta(over,d,quantile)
        cost,x,y,c,qv,E,z,E_day=mpc_day(price,load[d],pv[d],fc,d,E,delta)
        if d>=WARMUP_DAYS:
            ...
            e0=float(E_day[0]); e24=float(E_day[-1])
            E0_min=e0 if E0_min is None else min(E0_min,e0); E0_max=e0 if E0_max is None else max(E0_max,e0)
            E24_min=e24 if E24_min is None else min(E24_min,e24); E24_max=e24 if E24_max is None else max(E24_max,e24)
        emin=min(emin,float(E_day.min())); emax=max(emax,float(E_day.max()))
    ...
    return {'total':round(total,2),'plan':round(plan,2),'penalty':round(penalty,2),'emergency':round(emergency,2),
            ...
            'curtailment_rate':round(curt/pv_energy,6),
            'E_min':round(emin,2),'E_max':round(emax,2),
            'E0_min':round(E0_min,2),'E0_max':round(E0_max,2),
            'E24_min':round(E24_min,2),'E24_max':round(E24_max,2),
            'q':quantile}


def run_baselines(b):
    """块级 q=80 口径下的对比基线（与主模型同信息、同修正轴）。"""
    price=b['附件1']['price']; load=b['附件2_load']; pv=b['附件2_pv']; fc=b['附件3']['forecast']; D=len(load)
    over=block_over(fc,pv)
    b3=0.0; E=E0_INIT
    for d in range(D):
        delta=block_delta(over,d,80.0)
        cost,E,_=static_day_block(price,load[d],pv[d],fc,d,E,delta)
        if d>=WARMUP_DAYS: b3+=cost
    r3=0.0; E=E0_INIT
    for d in range(D):
        delta=block_delta(over,d,80.0)
        cost,E,_=rule_day_block(price,load[d],pv[d],fc,d,E,delta)
        if d>=WARMUP_DAYS: r3+=cost
    ns=0.0; ns_fc=0.0
    for d in range(WARMUP_DAYS,D):
        ns+=no_storage(price,load[d],pv[d])[0]
        ns_fc+=no_storage_dayahead(price,load[d],pv[d],fc,d)[0]
    return {'B3_block':round(b3,2),'R3_block':round(r3,2),
            'B3_ref_nostorage_perfect':round(ns,2),'NS_fc_nostorage_forecast':round(ns_fc,2)}
