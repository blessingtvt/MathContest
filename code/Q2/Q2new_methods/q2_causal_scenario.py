# -*- coding: utf-8 -*-
"""Q2 多场景随机规划（scenario stochastic programming）+ CVaR 尾部风险约束。

作为 round4 逐日滚动 LP 的随机规划扩展（论文稳健性/对照章节），用一组净负荷
误差场景刻画预报不确定性，并在目标中显式加入 CVaR 尾部风险项，替代 round4
的固定分位余量。信息集与 round4 完全一致：第 d 天 0:00 只使用 d 日之前的
附件2历史数据（时间因果、无前瞻），不使用完美预见、附件3、日内更新或全年
联合规划。

每天求解一个场景 LP：场景共享 x/c/q/E，场景独立紧急购电 z_s；目标 = 期望
购电费（含场景加权紧急费）+ rho·CVaR_β(当日总费用)。执行后再用当日真实净
负荷结算 z = max(N·Δt + c − x − q, 0)。
"""
import json, os, sys
